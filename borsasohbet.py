...
Bot her sabah radar, piyasa rüzgarı ve erken uyarı; akşam ise piyasa özeti, akıllı para ve BES özeti gönderebilir.

━━━━━━━━━━━━━━
🔔 SİNYAL ALARM / HATIRLATMA
━━━━━━━━━━━━━━
• sinyal alarm
• alarm durum
• sinyal kontrol
...
Kaydettiğin sinyallerde karar, skor, fiyat veya piyasa filtresi değişirse otomatik bildirim gönderirim.
...
def sinyal_kaydet_mesaji(hisse: str, chat_id=None):
    sinyal, hata = sinyal_uret(hisse)
    if hata:
        return hata

    if chat_id:
        sinyal["chat_id"] = chat_id
        sinyal["alarm_aktif"] = True
        sinyal["son_kontrol"] = None
        sinyal["son_alarm"] = None
...
Bu sinyal ileride başarı oranı ölçümünde kullanılacak.\nAyrıca karar/skor/fiyat değişirse otomatik hatırlatma gönderilecek."""
...
def sinyal_komutu_mu(metin: str):
    ...
    return any(k in m for k in [
        ...
        "performans", "sinyal alarm", "alarm durum",
        "sinyal kontrol", "takipteki sinyaller", "sinyal hatırlatma", "sinyal hatirlatma"
    ])
...
def sinyal_komutu_mesaji(metin: str, chat_id=None):
    m = temizle_metin(metin)

    if "sinyal alarm" in m or "alarm durum" in m or "sinyal kontrol" in m or "takipteki sinyaller" in m or "sinyal hatırlatma" in m or "sinyal hatirlatma" in m:
        return sinyal_alarm_durum_mesaji()
    ...
    if hisse and ("sinyal kaydet" in m or "sinyali kaydet" in m):
        return sinyal_kaydet_mesaji(hisse, chat_id=chat_id)
...
# =========================
# SİNYAL ALARM / DEĞİŞİM HATIRLATMA
# =========================

SINYAL_KONTROL_DAKIKA = int(os.getenv("SINYAL_KONTROL_DAKIKA", "30"))
SINYAL_SKOR_ESIK = int(os.getenv("SINYAL_SKOR_ESIK", "10"))
SINYAL_FIYAT_ESIK = float(os.getenv("SINYAL_FIYAT_ESIK", "3"))
...
def telegram_mesaj_gonder(chat_id, text):
    ...
    try:
        ...
        print(f"Telegram bildirim gönderilemedi: {e}", flush=True)
        return False


def sinyal_degisimlerini_bul(eski, yeni):
    degisimler = []

    eski_karar = eski.get("karar")
    yeni_karar = yeni.get("karar")
    if eski_karar and yeni_karar and eski_karar != yeni_karar:
        degisimler.append(f"Karar değişti: {eski_karar} → {yeni_karar}")

    eski_skor = eski.get("filtreli_skor")
    yeni_skor = yeni.get("filtreli_skor")
    if eski_skor is not None and yeni_skor is not None:
        fark = int(yeni_skor) - int(eski_skor)
        if abs(fark) >= SINYAL_SKOR_ESIK:
            yon = "arttı" if fark > 0 else "azaldı"
            degisimler.append(f"Skor {yon}: {eski_skor}/100 → {yeni_skor}/100")

    eski_fiyat = eski.get("fiyat")
    yeni_fiyat = yeni.get("fiyat")
    if eski_fiyat and yeni_fiyat:
        fiyat_fark = ((float(yeni_fiyat) - float(eski_fiyat)) / float(eski_fiyat)) * 100
        if abs(fiyat_fark) >= SINYAL_FIYAT_ESIK:
            yon = "yükseldi" if fiyat_fark > 0 else "düştü"
            degisimler.append(f"Fiyat %{fiyat_fark:+.2f} {yon}: {fmt(eski_fiyat)} TL → {fmt(yeni_fiyat)} TL")

    eski_piyasa = eski.get("piyasa_durum")
    yeni_piyasa = yeni.get("piyasa_durum")
    if eski_piyasa and yeni_piyasa and eski_piyasa != yeni_piyasa:
        degisimler.append(f"Piyasa filtresi değişti: {eski_piyasa} → {yeni_piyasa}")

    # TP / SL benzeri pratik uyarı
    if eski_fiyat and yeni_fiyat:
        getiri = ((float(yeni_fiyat) - float(eski_fiyat)) / float(eski_fiyat)) * 100
        if getiri >= 5 and not eski.get("tp_uyari_gonderildi"):
            degisimler.append(f"TP bölgesine yaklaştı/geçti: getiri %{getiri:+.2f}")
            eski["tp_uyari_gonderildi"] = True
        if getiri <= -3 and not eski.get("sl_uyari_gonderildi"):
            degisimler.append(f"Stop-loss bölgesine yaklaştı/geçti: getiri %{getiri:+.2f}")
            eski["sl_uyari_gonderildi"] = True

    return degisimler


def sinyal_alarm_mesaji_olustur(eski, yeni, degisimler):
    hisse = eski.get("hisse", yeni.get("hisse", "-"))
    return f"""🚨 Sinyal Değişim Uyarısı

{hisse}

{chr(10).join("- " + d for d in degisimler)}

Güncel Durum:
{yeni.get('emoji','')} Karar: {yeni.get('karar','-')}
Skor: {yeni.get('filtreli_skor','-')}/100
Güven: %{yeni.get('guven','-')}
Fiyat: {fmt(yeni.get('fiyat'))} TL
Piyasa: {yeni.get('piyasa_durum','-')} ({yeni.get('piyasa_skor','-')}/100)

Komut:
{hisse}"""


def sinyal_alarm_kontrol_et():
    data = json_yukle(SINYAL_DOSYA, [])
    if not data:
        return
    ...
    # En yeni açık sinyaller önce; Render ücretsiz kaynakları için çok fazla taramıyoruz.
    for idx, eski in list(enumerate(data))[-25:]:
        try:
            if eski.get("alarm_kapali"):
                continue
            if eski.get("durum") not in [None, "açık", "acik"]:
                continue

            chat_id = eski.get("chat_id")
            if not chat_id:
                continue

            hisse = eski.get("hisse")
            if not hisse:
                continue

            yeni, hata = sinyal_uret(hisse)
            if hata or not yeni:
                continue

            degisimler = sinyal_degisimlerini_bul(eski, yeni)
            eski["son_kontrol"] = simdi

            if degisimler:
                mesaj = sinyal_alarm_mesaji_olustur(eski, yeni, degisimler)
                gitti = telegram_mesaj_gonder(chat_id, mesaj)
                if gitti:
                    eski["son_alarm"] = simdi
                    eski["son_alarm_detay"] = degisimler

                # Baz değerleri güncelle; aynı değişim tekrar tekrar bildirilmesin.
                eski["fiyat"] = yeni.get("fiyat")
                eski["teknik_skor"] = yeni.get("teknik_skor")
                eski["ham_skor"] = yeni.get("ham_skor")
                eski["filtreli_skor"] = yeni.get("filtreli_skor")
                eski["guven"] = yeni.get("guven")
                eski["karar"] = yeni.get("karar")
                eski["emoji"] = yeni.get("emoji")
                eski["piyasa_skor"] = yeni.get("piyasa_skor")
                eski["piyasa_durum"] = yeni.get("piyasa_durum")
                eski["bekle_nedenleri"] = yeni.get("bekle_nedenleri", [])
                degisti = True
            else:
                degisti = True

        except Exception as e:
            print(f"Sinyal alarm kontrol hata: {e}", flush=True)

    if degisti:
        json_kaydet(SINYAL_DOSYA, data)


def sinyal_alarm_dongusu():
    print(f"Sinyal alarm sistemi aktif. Kontrol aralığı: {SINYAL_KONTROL_DAKIKA} dk", flush=True)
    while True:
        try:
            sinyal_alarm_kontrol_et()
        except Exception as e:
            print(f"Sinyal alarm döngüsü hata: {e}", flush=True)
        time.sleep(max(5, SINYAL_KONTROL_DAKIKA) * 60)


def start_sinyal_alarm_thread():
    t = threading.Thread(target=sinyal_alarm_dongusu, daemon=True)
    t.start()


def sinyal_alarm_durum_mesaji():
    data = json_yukle(SINYAL_DOSYA, [])
    aktif = [s for s in data if s.get("chat_id") and not s.get("alarm_kapali") and s.get("durum") in [None, "açık", "acik"]]

    if not aktif:
        return """🔔 Sinyal Alarm Durumu
...
Kaydedilen sinyaller otomatik takip edilir. Karar, skor, fiyat veya piyasa filtresi değişirse bildirim gönderirim."""

    satirlar = []
    for i, s in enumerate(reversed(aktif[-10:]), 1):
        satirlar.append(
            f"{i}. {s.get('emoji','')} {s.get('hisse')} | {s.get('karar')} | "
            f"Skor {s.get('filtreli_skor')}/100 | Fiyat {fmt(s.get('fiyat'))} TL"
        )

    return f"""🔔 Sinyal Alarm Durumu

Kontrol Aralığı: {SINYAL_KONTROL_DAKIKA} dk
Skor Eşiği: {SINYAL_SKOR_ESIK} puan
Fiyat Eşiği: %{SINYAL_FIYAT_ESIK}

Aktif Takip:
{chr(10).join(satirlar)}"""


def sinyal_alarm_komutu_mu(metin: str):
    m = temizle_metin(metin)
    return any(k in m for k in [
        "sinyal alarm", "alarm durum", "sinyal kontrol",
        "takipteki sinyaller", "sinyal hatırlatma", "sinyal hatirlatma"
    ])


def sinyal_alarm_komutu_mesaji(metin: str):
    return sinyal_alarm_durum_mesaji()
...
# =========================
# SABAH / AKŞAM OTOMATİK BÜLTEN
# =========================

BULTEN_DOSYA = DATA_DIR / "bulten_ayar.json"
BULTEN_SABAH_SAAT = os.getenv("BULTEN_SABAH_SAAT", "08:30")
BULTEN_AKSAM_SAAT = os.getenv("BULTEN_AKSAM_SAAT", "18:30")


def bulten_ayar_yukle():
    return json_yukle(BULTEN_DOSYA, {"chat_ids": [], "son_gonderim": {}})


def bulten_ayar_kaydet(data):
    json_kaydet(BULTEN_DOSYA, data)


def bulten_abone_ekle(chat_id):
    data = bulten_ayar_yukle()
    chat_ids = data.get("chat_ids", [])
    if chat_id not in chat_ids:
        chat_ids.append(chat_id)
        data["chat_ids"] = chat_ids
        bulten_ayar_kaydet(data)
    return True


def bulten_abone_sil(chat_id):
    data = bulten_ayar_yukle()
    chat_ids = data.get("chat_ids", [])
...
def sabah_bulteni_mesaji():
    bolumler = ["🌅 Sabah Piyasa Bülteni"]

    try:
        bolumler.append(piyasa_ruzgari_mesaji())
    except Exception as e:
        bolumler.append(f"🌊 Piyasa rüzgarı hazırlanamadı: {e}")

    try:
        bolumler.append(radar_mesaji())
    except Exception as e:
        bolumler.append(f"📡 Radar hazırlanamadı: {e}")

    try:
        bolumler.append(erken_uyari_mesaji())
    except Exception as e:
        bolumler.append(f"⚠️ Erken uyarı hazırlanamadı: {e}")

    return "\n\n".join(bolumler).strip()


def aksam_bulteni_mesaji():
    bolumler = ["🌙 Akşam Piyasa Özeti"]

    try:
        bolumler.append(bugun_piyasada_ne_oluyor_mesaji())
    except Exception:
        try:
            bolumler.append(piyasa_ruzgari_mesaji())
        except Exception as e:
            bolumler.append(f"🧩 Piyasa özeti hazırlanamadı: {e}")

    try:
        bolumler.append(akilli_para_mesaji())
    except Exception as e:
        bolumler.append(f"🦈 Akıllı para özeti hazırlanamadı: {e}")

    try:
        bolumler.append(bes_mesaji("BES haberleri"))
    except Exception as e:
        bolumler.append(f"🏦 BES özeti hazırlanamadı: {e}")

    return "\n\n".join(bolumler).strip()


def saat_eslesti_mi(hedef_saat):
    try:
        now = datetime.now()
        h, m = hedef_saat.split(":")
        return now.hour == int(h) and now.minute == int(m)
    except Exception:
        return False


def bulten_gonder(tip):
    data = bulten_ayar_yukle()
    chat_ids = data.get("chat_ids", [])
    if not chat_ids:
        return
    ...
    key = f"{tip}_{bugun}"
    son = data.get("son_gonderim", {})
    if son.get(key):
        return

    if tip == "sabah":
        mesaj = sabah_bulteni_mesaji()
    else:
        mesaj = aksam_bulteni_mesaji()

    for chat_id in chat_ids:
        telegram_mesaj_gonder(chat_id, mesaj)

    son[key] = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    data["son_gonderim"] = son
    bulten_ayar_kaydet(data)


def bulten_dongusu():
    print(f"Otomatik bülten aktif. Sabah: {BULTEN_SABAH_SAAT}, Akşam: {BULTEN_AKSAM_SAAT}", flush=True)
    while True:
        try:
            if saat_eslesti_mi(BULTEN_SABAH_SAAT):
                bulten_gonder("sabah")
            if saat_eslesti_mi(BULTEN_AKSAM_SAAT):
                bulten_gonder("aksam")
        except Exception as e:
            print(f"Bülten döngüsü hata: {e}", flush=True)
        time.sleep(30)


def start_bulten_thread():
    t = threading.Thread(target=bulten_dongusu, daemon=True)
    t.start()


def bulten_komutu_mu(metin: str):
    m = temizle_metin(metin)
    return any(k in m for k in [
        "bülten", "bulten", "sabah bülteni", "sabah bulteni",
        "akşam özeti", "aksam ozeti", "akşam bülteni", "aksam bulteni",
        "günlük bülten", "gunluk bulten", "otomatik bülten", "otomatik bulten"
    ])


def bulten_komutu_mesaji(metin: str, chat_id=None):
    m = temizle_metin(metin)

    if "kapat" in m or "iptal" in m or "dur" in m:
        if chat_id:
            bulten_abone_sil(chat_id)
        return "🔕 Otomatik sabah/akşam bülteni kapatıldı."

    if "aç" in m or "ac" in m or "başlat" in m or "baslat" in m or "aktif" in m:
        if chat_id:
            bulten_abone_ekle(chat_id)
        return f"""🔔 Otomatik bülten açıldı.

Sabah Bülteni: {BULTEN_SABAH_SAAT}
Akşam Özeti: {BULTEN_AKSAM_SAAT}

Kapatmak için:
bülten kapat"""

    if "sabah" in m:
        return sabah_bulteni_mesaji()

    if "akşam" in m or "aksam" in m:
        return aksam_bulteni_mesaji()

    data = bulten_ayar_yukle()
    aktif = chat_id in data.get("chat_ids", []) if chat_id else False
    durum = "Açık ✅" if aktif else "Kapalı ❌"

    return f"""🔔 Sabah / Akşam Bülteni
...
Saatler:
• Sabah: {BULTEN_SABAH_SAAT}
• Akşam: {BULTEN_AKSAM_SAAT}

Saatleri Render Environment üzerinden değiştirebilirsin:
BULTEN_SABAH_SAAT=08:30
BULTEN_AKSAM_SAAT=18:30"""
...
async def mesaj_yakala(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if bulten_komutu_mu(metin):
        await update.message.reply_text("Bülten ayarına bakıyorum...")
        try:
            mesaj = bulten_komutu_mesaji(metin, chat_id=update.effective_chat.id)
            await update.message.reply_text(mesaj[:3900], disable_web_page_preview=True)
        except Exception as e:
            await update.message.reply_text(f"Bülten işlemi sırasında hata oluştu: {e}")
        return

    if sinyal_alarm_komutu_mu(metin):
        try:
            mesaj = sinyal_alarm_komutu_mesaji(metin)
            await update.message.reply_text(mesaj[:3900], disable_web_page_preview=True)
        except Exception as e:
            await update.message.reply_text(f"Sinyal alarm durumu alınırken hata oluştu: {e}")
        return
    ...
    if sinyal_komutu_mu(metin):
        ...
        try:
            mesaj = sinyal_komutu_mesaji(metin, chat_id=update.effective_chat.id)
            await update.message.reply_text(mesaj[:3900], disable_web_page_preview=True)
        except Exception as e:
